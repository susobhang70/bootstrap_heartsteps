...
def aggComputeMetricSlidingWindow(result):
    ...
    metric += res['interestingnessStatistic-SlidingWindow']['isInteresting']
...
def main():
    ...
    print(len(bootstrapped_results))

    # Run algorithm
    # image_path=os.path.join(args.output, "user_"+args.baseline)
    result=getTableInterestingness(original_result[args.user], bootstrapped_results)        
    print(result)
